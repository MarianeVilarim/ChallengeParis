...
@available(iOS 13, *)
struct GeraCodigoView_Preview: PreviewProvider {
    static var previews: some View {
        // view controller using programmatic UI
        Group {
            GeraCodigoView().showPreview().previewDevice("iPhone SE (3rd generation)")
            GeraCodigoView().showPreview().previewDevice("iPhone SE (3rd generation)").previewInterfaceOrientation(.landscapeLeft)
        }
    }
}
#endif
